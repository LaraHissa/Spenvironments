 ...
 *  Provides utility methods for data export, visualization, and statistical analysis.
 *********************************************************************************************************/
 ...
 class auxiliar_functions {
    public:
        ...
        void save_phe_genomes(speciation_model &spec_model, const string& filename);
...
void auxiliar_functions::save_phe_genomes(speciation_model &spec_model, const string& filename) {
    vector<vector<int>> PheGenome = spec_model.get_phegenome();
    ofstream outFile(filename);
    if (!outFile.is_open()) return;

    for (const auto& genome : PheGenome) {
        for (int bit : genome) outFile << bit;
        outFile << endl;
    }
    outFile.close();
